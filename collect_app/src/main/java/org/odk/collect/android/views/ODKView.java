...
public class ODKView extends ScrollView implements OnLongClickListener {
    ...
    private String TAG="ODKView";
    public static final String FIELD_LIST = "field-list";

    public ODKView(Context context, final FormEntryPrompt[] questionPrompts,
                   FormEntryCaption[] groups, boolean advancingPage, final HashMap<String,HashMap<String,String>> dataSensMap) {
        super(context);
        ...
        for (FormEntryPrompt p : questionPrompts) {
            ...
            String qtnText = p.getQuestion().getTextID().split(":")[0];
            HashMap<String,String> profile=dataSensMap.get(qtnText);
            AttributeProfile attributeProfile= new AttributeProfile(qtnText,profile);
            // if question or answer type is not supported, use text widget
            QuestionWidget qw =
                    WidgetFactory.createWidgetFromPrompt(p, getContext(), readOnlyOverride,attributeProfile);
            Timber.i(TAG+" profile is %s", profile);
