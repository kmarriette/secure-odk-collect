        ...
        switch (event) {
            ...
            case FormEntryController.EVENT_REPEAT:
                ...
                try {
                    ...
                    ArrayList sdfile = FileUtils.readSDfile(formXml);

                    HashMap<String,String> sens = FileUtils.getSensitivity(formXml);

                    /*ArrayList<String> keyList = new ArrayList<String>(sens.keySet());
                    ArrayList<String> valueList = new ArrayList<String>(sens.values());

                    Timber.i("sensitivity keys %s", keyList);
                    Timber.i("sensitivity values %s", valueList);*/
                    ArrayList<AttributeProfile> eList= new ArrayList();
                    HashMap<String,HashMap<String,String>>  dataSensMap = new HashMap<>();
                    //compare prompt p with sens key, then obtain the security mechanism from the sdfile
                    for (int x = 0; x < prompts.length; x++){
                        String qtnText = prompts[x].getQuestion().getTextID().split(":")[0];

                        Timber.i("qtnText %s", qtnText);

                        String level = sens.get(qtnText);

                        Timber.i("level %s", level);

                        HashMap<String,String> profile = new HashMap<>();

                        for (int s = 0; s < sdfile.size(); s++){
                            profile = (HashMap<String, String>) sdfile.get(s);

                            String profileLevel=profile.get("level");
                            if(level.equals(profileLevel)){
                                dataSensMap.put(qtnText,profile);
                                AttributeProfile elementSensitivity=new AttributeProfile(qtnText,profile);
                                eList.add(elementSensitivity);
                                break;
                            }
                        }

                        //ArrayList<String> keyList = new ArrayList<String>(profile.keySet());
                        //ArrayList<String> valueList = new ArrayList<String>(profile.values());

                        //Timber.i("profile keys %s", keyList);
                        //Timber.i("profile values %s", valueList);
                    }


                    odkv = new ODKView(this, prompts, groups, advancingPage,dataSensMap);
                    //Timber.i("Prompt p1 is: %s", prompts[0].getQuestion().getTextID().split(":")[0]);
                    //Timber.i("Prompt p2 is: %s", prompts[0].getQuestionText());
                    //Timber.i("Prompt p2 is: %s", prompts[0].getQuestion().);

                    //MKK
                    //final String formPath = formPath[0];
