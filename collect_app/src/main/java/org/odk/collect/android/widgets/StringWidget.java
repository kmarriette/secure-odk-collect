...
import org.odk.collect.android.sens.MaskFilter;
...
public class StringWidget extends QuestionWidget {
    ...
    private String TAG="StringWidget";
    private AttributeProfile attributeProfile;
    boolean readOnly = false;

    public StringWidget(Context context, FormEntryPrompt prompt, boolean readOnlyOverride,AttributeProfile attributeProfile) {
        this(context, prompt, readOnlyOverride, true,attributeProfile);
        setupChangeListener();
    }

    protected StringWidget(Context context, FormEntryPrompt prompt, boolean readOnlyOverride,
                           boolean derived, AttributeProfile attributeProfile) {
        super(context, prompt,attributeProfile);
    ...
    public void setMasking(){
        String masking=attributeProfile.getMasking();
        Timber.i(TAG +" Masking= %s",masking);
        if(masking!=null){
            if(masking.equals(SensLogic.YES)){
                Timber.i(TAG +" Now Masking %s",masking);
                InputFilter filter = MaskFilter.getFilter();
                answerText.setFilters(new InputFilter[] { filter });
            }
        }
    }
